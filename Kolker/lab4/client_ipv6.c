...
void main()
{
 struct sockaddr_in6 sockaddr; //Структура для хранения адреса

 udp_socket = socket(AF_INET6, SOCK_DGRAM, 0); // Инициализация сокета
    if(udp_socket < 0)
    {
        perror("socket");
        exit(1);
 ...
 buf_send[81]=(char)xorbyte; //Контрольный
