...
public:
	...
	{
		array = new T[src.size]; // создаем новый массив, исходя из размера исходного объекта
		size = src.size; // просто копируем поля исходного об. в создаваемый
		index = src.index;
		for (int i = 0; i < size; i++)// просто копируем поля исходного об. в создаваемый
		{
			array[i] = src.array[i];
		}
	}
